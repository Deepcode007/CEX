...
while (true) {
    ...
    for (let items of messages) {
        console.log(items.message.data);
        ...
        if (parsed.reason == "CREATE_ORDER") {
            ...
            console.log(data);
            let abs = await prisma.order.create({
                data: data,
            });
            console.log(abs);
            ...
            await prisma.$transaction([
                ...
                prisma.fills.create({
                    data: {
                        ...
                        type: parsed.type,
                        status:
                            parsed.filled_quantity === parsed.quantity
                                ? "filled"
                                : "open",
                        createdAt: parsed.createdAt,
                    },
            ...
            if (
                !wallet ||
                (parsed.reason == "WITHDRAW" && wallet.balance < parsed.delta)
            )
                continue;
            await prisma.$transaction([
                prisma.wallet.update({
