...
export async function UpdateToRedis(redis_dumper: RedisClientPoolType, makerOrder: Orders, takerOrder: Orders, req_qty: number) {
  ...
  if (takerOrder.side == "bids") {
    ...
    let c = dump(redis_dumper, takerOrder, req_qty, takerOrder.asset, "maker", "PROCESS_ORDER");
    // decrease my asset
    let d = dump(redis_dumper, makerOrder, -req_qty, takerOrder.asset, "taker", "PROCESS_ORDER");
